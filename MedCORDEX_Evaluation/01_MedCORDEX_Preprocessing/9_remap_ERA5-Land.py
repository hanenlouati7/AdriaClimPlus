### remap MEDCORDEX data over the ERA5-Land grid since MEDCORDEX data is finer than ERA5-Land
#ERA5 --> 0.1 deg
#ERA 5 - Land --> 0.25 deg
...
for yr in yr_v:
    for i in range(len(seasons)):
        input_file_era5land_t2m = f"../../data_ERA5-Land/processed/ERA5-Land_{yr}_t2m_{seasons[i]}.nc"
...
for start_yr, end_yr in steps:
    for i in range(len(seasons)):
        input_file_era5land_t2m = f"../../data_ERA5-Land/processed/ERA5-Land_{start_yr}_{end_yr}_t2m_{seasons[i]}.nc"
...
for yr in yr_v:
    for i in range(len(seasons)):
        input_file_era5land_tp = f"../../data_ERA5-Land/processed/ERA5-Land_{yr}_tp_{seasons[i]}.nc"
...
for start_yr, end_yr in steps:
    for i in range(len(seasons)):
        input_file_era5land_tp = f"../../data_ERA5-Land/processed/ERA5-Land_{start_yr}_{end_yr}_tp_{seasons[i]}.nc"
...
for yr in yr_v:
    for i in range(0,12):
        input_file_era5land_vn = f"../../data_ERA5-Land/processed/ERA5-Land_{yr}_vn_{months[i]}.nc"
...
for start_yr, end_yr in steps:
    for i in range(0,12):
        input_file_era5land_vn = f"../../data_ERA5-Land/processed/ERA5-Land_{start_yr}_{end_yr}_vn_{months[i]}.nc"
...
for yr in yr_v:
    for i in range(len(seasons)):
        input_file_era5land_stru = f"../../data_ERA5-Land/processed/ERA5-Land_{yr}_stru_{seasons[i]}.nc"
        ...
        command = f"cdo -remapnn,{input_file_era5land_stru} {input_file_medcordex_stru} {output_file_medcordex_stru}"
        try:
            subprocess.run(command, shell=True, check=True)
        except subprocess.CalledProcessError as e:
            print(f"Error while processing {yr} for part {i}: {e}")
...
for start_yr, end_yr in steps:
    for i in range(len(seasons)):
        input_file_era5land_stru = f"../../data_ERA5-Land/processed/ERA5-Land_{start_yr}_{end_yr}_stru_{seasons[i]}.nc"
...
for yr in yr_v:
    for i in range(len(seasons)):
        input_file_era5land_ssrd = f"../../data_ERA5-Land/processed/ERA5-Land_{yr}_ssrd_{seasons[i]}.nc"
        input_file_medcordex_ssrd = f"../processed/MEDCORDEX_{yr}_ssrd_{seasons[i]}.nc"
        output_file_medcordex_ssrd = f"../processed/MEDCORDEX_{yr}_ssrd_remap_{seasons[i]}.nc"
        command = f"cdo -remapnn,{input_file_era5land_ssrd} {input_file_medcordex_ssrd} {output_file_medcordex_ssrd}"
        try:
            subprocess.run(command, shell=True, check=True)
        except subprocess.CalledProcessError as e:
            print(f"Error while processing {yr} for part {i}: {e}")
...
for start_yr, end_yr in steps:
    for i in range(len(seasons)):
        input_file_era5land_ssrd = f"../../data_ERA5-Land/processed/ERA5-Land_{start_yr}_{end_yr}_ssrd_{seasons[i]}.nc"
        input_file_medcordex_ssrd = f"../processed/MEDCORDEX_{start_yr}_{end_yr}_ssrd_{seasons[i]}.nc"
        output_file_medcordex_ssrd = f"../processed/MEDCORDEX_{start_yr}_{end_yr}_ssrd_remap_{seasons[i]}.nc"
        command = f"cdo -remapnn,{input_file_era5land_ssrd} {input_file_medcordex_ssrd} {output_file_medcordex_ssrd}"
        try:
            subprocess.run(command, shell=True, check=True)
        except subprocess.CalledProcessError as e:
