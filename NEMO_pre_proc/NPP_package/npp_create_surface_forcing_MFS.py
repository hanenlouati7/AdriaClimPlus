...
for month in range(sf_monthini,sf_monthend+1):
    ...
    for day in range(1,daye+1):
        ...
        timef=[] ; mslf=[] ; tccf=[] ; u10f=[]
        v10f=[] ; t2mf=[] ; d2mf=[] ; precipf=[]
        for hour in range(0,18+1,6):
            ...
            lonsf,latsf,time,msl,tcc,u10,v10,t2m,d2m,lsm,precip = load_input_surface_forcings_MFS(final_sf_filename,dt)
## Add time counter
            timef.append(dt*6)
...
## Check if the latitude is inverted
            if(latsf[0]>latsf[len(latsf)-1]):
                ...
                tcc=np.fliplr([tcc])[0] ; u10=np.fliplr([u10])[0]
                v10=np.fliplr([v10])[0] ; t2m=np.fliplr([t2m])[0]
                d2m=np.fliplr([d2m])[0] ; precip=np.fliplr([precip])[0]
##################################################################
...
## Seaoverland extrapolation fields
            ...
            tccf.append(sf_seaoverland(tcc))     # Total cloud cover
...
##
        mslf=np.asarray(mslf) ; tccf=np.asarray(tccf)
...
#        print('Write Surface Forcing netcdf output file')
        sf_output_filenamef=sf_output_folder+sf_output_filename+\
                            '_y'+ny+'m'+nm+'d'+nd+'.nc'
        oSFFile = write_SF_nc_output_MFS(lonsf, latsf, ny, nm, nd, timef, mslf,\
                  tccf, u10f, v10f, t2mf, d2mf, precipf, sf_output_filenamef)
###
        del timef, mslf, tccf, u10f, v10f, t2mf, d2mf, precipf
##################################################################
print(' ')
