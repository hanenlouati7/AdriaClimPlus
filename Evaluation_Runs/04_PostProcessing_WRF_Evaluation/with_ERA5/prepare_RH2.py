...
process_path = "/work/cmcc/vr25423/Project/AdriaClimPlus/evaluation_run_30yrs_ERA5/pp/ERA5/files_RH2"
### RH2 Average ###
...
for yr in yr_v:
    for mn in mn_v:
        ...
        for dt in dt_v:
            ...
            output_file = f"{process_path}/del_RH2_wrfout_d01_{yr}-{mn}-{dt}_00:00:00"
            command = f"cdo selvar,Q2,PSFC,T2 {input_file} {output_file}"
            try:
                subprocess.run(command, shell=True, check=True)
            except subprocess.CalledProcessError as e:
                print(f"Error while processing {yr}: {e}")
            ### Calculating RH2 ###
            input_file = f"{process_path}/del_RH2_wrfout_d01_{yr}-{mn}-{dt}_00:00:00"
            output_file = f"{process_path}/del_RH2_calc_wrfout_d01_{yr}-{mn}-{dt}_00:00:00"
            command = f'cdo expr,"RH2=Q2/((379.90516/PSFC)*exp(17.2693882*(T2-273.16)/(T2-35.86)))" {input_file} {output_file}'
            try:
                subprocess.run(command, shell=True, check=True)
            except subprocess.CalledProcessError as e:
                print(f"Error while processing {yr}: {e}")
            ### Remapping ###
            input_file = f"{process_path}/del_RH2_calc_wrfout_d01_{yr}-{mn}-{dt}_00:00:00"
            remap_ref_file = f"{era5_path}/ERA5_1990_unpacked.nc"
            remapped_output_file = f"{process_path}/del_remapped_RH2_wrfout_d01_{yr}-{mn}-{dt}_00:00:00"
    ...
    ### Merging ###
    file_pattern = os.path.join(process_path, f"del_remapped_RH2_wrfout_d01_{yr}-*-*_00:00:00")
    files = sorted(glob(file_pattern))
    output_file = f"{process_path}/del_remapped_RH2_wrfout_d01_{yr}.nc"
    ...
    for i, (start, end) in enumerate(steps, 1):
        input_file = f"{process_path}/del_remapped_RH2_wrfout_d01_{yr}.nc"
        output_file = f"{process_path}/del_remapped_RH2_wrfout_d01_{yr}"
        ...
        input_file = f"{process_path}/del_remapped_RH2_wrfout_d01_{yr}_all_{i}.nc"
        output_file = f"{process_path}/RH2_wrfout_d01_{yr}"
        ### Seasonal mean ###
