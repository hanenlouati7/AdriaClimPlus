...
process_path = "/work/cmcc/vr25423/Project/AdriaClimPlus/evaluation_run_30yrs_ERA5/pp/ERA5/files_tp"
### Precipitation ###
...
for yr in yr_v:
    for mn in mn_v:
        ...
        for dt in dt_v:
            ### Extracting PREC variables ###
            input_file = f"{wrfout_path}/wrfout_d01_{yr}-{mn}-{dt}_00:00:00"
            output_file = f"{process_path}/del_prec_wrfout_d01_{yr}-{mn}-{dt}_00:00:00"
            command = f"cdo selvar,RAINC,RAINNC,I_RAINC,I_RAINNC {input_file} {output_file}"
            try:
                subprocess.run(command, shell=True, check=True)
            except subprocess.CalledProcessError as e:
                print(f"Error while processing {yr}: {e}")
            ### Remapping ###
            input_file = f"{process_path}/del_prec_wrfout_d01_{yr}-{mn}-{dt}_00:00:00"
            remap_ref_file = f"{era5_path}/ERA5_1990_unpacked.nc"
            remapped_output_file = f"{process_path}/del_remapped_prec_wrfout_d01_{yr}-{mn}-{dt}_00:00:00"
