...
print(' ')
lonts,latts,zts,tempi,sali = load_input_initial_TS_MedCordex(tempi_filename,sali_filename)
##################################################################
...
print(' ')
mm=0 ; nn=0
for i in range(0,len(lonts[:,0])):
    lontsr=np.squeeze(lonts[i,:])
    if(np.min(lontsr)>np.min(lonnm)):
        mm=mm+1
        lontsr[lontsr==np.min(lontsr)]=np.min(lonnm)
        lonts[i,:]=lontsr
    if(mm==1): print('**Min. TS input file longitude > min. longitude NEMO new gridi - EXTRAPOLATION DONE!!') ; print(' ')
    lontsr=np.squeeze(lonts[i,:])
    if(np.max(lontsr)<np.max(lonnm)):
        nn=nn+1
        lontsr[lontsr==np.max(lontsr)]=np.max(lonnm)
        lonts[i,:]=lontsr
    if(nn==1): print('**Max. TS input file longitude < max. longitude NEMO new grid - EXTRAPOLATION DONE!!') ; print(' ')
mm=0 ; nn=0
for j in range(0,len(lonts[0,:])):
    lattsr=np.squeeze(latts[:,j])
    if(np.min(lattsr)>np.min(latnm)):
        mm=mm+1
        lattsr[lattsr==np.min(lattsr)]=np.min(latnm)
        latts[:,j]=lattsr
    if(mm==1): print('**Min. TS input file latitude > min.latitude NEMO new grid - EXTRAPOLATION DONE!!') ; print(' ')
    lattsr=np.squeeze(latts[:,j])
    if(np.max(lattsr)<np.max(latnm)):
        nn=nn+1
        lattsr[lattsr==np.max(lattsr)]=np.max(latnm)
        latts[:,j]=lattsr
    if(nn==1): print('**Max. TS input file latitude < max. latitude NEMO new grid - EXTRAPOLATION DONE!!') ; print(' ')
...
print(' ')
tempnm = hinterp_MedCordex_to_NEMO(lonts,latts,zts,tempi,lonnm,latnm)
#tempnm = extrapolation_3d(tempnm)
print('Horizontal interpolation SAL. to NEMO grid')
print(' ')
salnm  = hinterp_MedCordex_to_NEMO(lonts,latts,zts,sali,lonnm,latnm)
#salnm  = extrapolation_3d(salnm)
#################################################################
