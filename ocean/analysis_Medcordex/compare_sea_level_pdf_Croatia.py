...
##
station = 'DUBROVNIK'
lon_tide_gauge = 18.067
lat_tide_gauge = 42.656
tide_correction = 0.001 # convert to m if necessary
date_format = '%d-%m-%y' # '%Y-%m-%dT%H:%M:%SZ' # '%d-%m-%y'
##
print("Station = ",station)

# File paths
tide_gauge_file = f"insitu_monthly/processed/SSH_{station}_MONTHLY.txt"
mask_file = "/work/cmcc/resm-dev/vladimir/FLAME/EXPS/EvalRUN_03s/bathy_mask_03s/mesh_mask_03s.nc"

# Load tide gauge data
tide_gauge_df = pd.read_csv(tide_gauge_file, sep=r'\s+', header=0, skiprows=1)
tide_gauge_df.columns = ['datetime', 'sea_level']  # Rename columns for easier access

# Convert the DateTime column to datetime format
tide_gauge_df['datetime'] = pd.to_datetime(tide_gauge_df['datetime'], format=date_format)
tide_gauge_df['sea_level'] = tide_gauge_df['sea_level']*tide_correction # apply units correction
tide_gauge_df.set_index('datetime', inplace=True)
...
model_df = model_df.resample("ME").mean()
...
bins = np.linspace(valid_data["sea_level"].min(), valid_data["sea_level"].max(), 50)
